...
void detect_two_array()
{
	int i=0,j=0;
	char op[expr_size],fuc1[expr_size],fuc2[expr_size],op1[expr_size],op2[expr_size],op3[expr_size],op4[expr_size];
	num_v=0;
	for(i=0;i<num_inst-4;i++)
	{
		sscanf(inst[i],"%s %s %s",op,fuc1,fuc2);//数组的格式mul，add，add
		sscanf(inst[i+1],"%s %s %s",op1,fuc1,fuc2);
		sscanf(inst[i+2],"%s %s %s",op2,fuc1,fuc2);
		sscanf(inst[i+3],"%s %s %s",op3,fuc1,fuc2);
		sscanf(inst[i+4],"%s %s %s",op4,fuc1,fuc2);
		if(strcmp(op,"mul")==0&&strcmp(op1,"add")==0&&strcmp(op2,"add")==0&&strcmp(op3,"mul")==0&&strcmp(op4,"add")==0)
		{
			transform_two_array(i);
			i=i+4;
		}
	}
...
bool simple_expr(char op[])//判断是否为简单运算指令
{
	...
	if(strcmp(op,"div")==0||strcmp(op,"mul")==0)
		return true;
	if(strcmp(op,"mod")==0||strcmp(op,"array2")==0)
		return true;
	return false;
}
